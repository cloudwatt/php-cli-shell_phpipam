<?php
	class REST
	{
		...
		public function post(array $datas = null)
		{
			$this->_setOpt(CURLOPT_FAILONERROR, false);
			$this->_setOpt(CURLOPT_RETURNTRANSFER, true);
			...
			if($datas !== null && Tools::is('array&&count>0', $datas)) {
				$this->_setOpt(CURLOPT_POSTFIELDS, $datas);
			}

			return $this->_prepareUri()->_exec();
		}

		public function put(array $datas)
		{
			$this->_setOpt(CURLOPT_FAILONERROR, false);
			$this->_setOpt(CURLOPT_RETURNTRANSFER, true);
		...
		protected function _exec()
		{
			...
			if(($error = $this->getError()) !== false)
			{
				if($this->_debug) {
					...
					print_r(curl_getinfo($this->_handle));
				}
		...
		public function getError()
		{
			...
			elseif($errno !== 0) {
				return "PHP curl command return ".$errno." error code";
			}
			elseif($httpCode !== 200) {
				return "HTTP request return ".$httpCode." error code";
			}
			else {
				return false;
			}
