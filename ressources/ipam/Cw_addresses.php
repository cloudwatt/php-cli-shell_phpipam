 ...
 *
 */
class Cw_addresses_controller extends Common_api_functions  {
	...
	public function GET () {
		...
		elseif (@$this->_params->id=="search") {
			...
			{
				$where = array();
				$searchOptions = array('ip_addr', 'ip_version', 'dns_name', 'description', 'port', 'note', 'subnet_id');
				...
				foreach($searchOptions as $searchOption)
				{
					if(isset($this->_params->{$searchOption}))
					{
						...
						{
							case 'ip_addr':
								$param = $this->Subnets->transform_address($param, "decimal");
								break;
							case 'ip_version':
								if($param == 4) $operator = '<=';
						...
						switch(true)
						{
							case (preg_match('/^(##present##)$/i', $param)):
								if(!$not) {
									$where[] = $searchOption.' != ""';
									$where[] = $searchOption.' IS NOT NULL';
								}
								else {
									$where[] = '('.$searchOption.' = "" OR '.$searchOption.' IS NULL)';
								}
								continue(2);
...
//print_r($values);
//$this->Response->throw_exception(400, $query);
				$result = $this->Database->getObjectsQuery($query, $values);
		
				// check result
				if($result===false)					{ $this->Response->throw_exception(200, 'Addresses not found'); }
				else							{ return array("code"=>200, "data"=>$this->prepare_result ($result, $this->_params->controller, true, true)); }
			}
		}
        // search host ?
